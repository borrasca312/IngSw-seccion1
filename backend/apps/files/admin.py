...
class FileAdmin(admin.ModelAdmin):
    list_display = ('name', 'file_type', 'size_formatted', 'uploaded_by', 'created_at', 'file_actions')
    ...
    def file_actions(self, obj):
        ...
        if obj.file:
            # Descargar archivo
            actions.append(f'<a href="{obj.file.url}" target="_blank" title="Descargar" style="text-decoration:none;">📁</a>')
            
            # Ver archivo si es imagen
            if obj.file_type in ['image', 'pdf']:
                actions.append(f'<a href="{obj.file.url}" target="_blank" title="Ver archivo" style="text-decoration:none;">👁️</a>')
