...
class UserAdmin(BaseUserAdmin):
    list_display = ('username', 'email', 'first_name', 'last_name', 'rut', 'rama', 'is_active', 'date_joined', 'user_actions')
    ...
    fieldsets = (
        ...
        ('Información Personal', {
            'fields': ('first_name', 'last_name', 'email', 'rut', 'phone', 'birth_date')
        }),
        ('Información Scout', {
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('username', 'email', 'password1', 'password2', 'first_name', 'last_name', 'rut'),
        }),
    )
